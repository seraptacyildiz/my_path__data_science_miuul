...
df = pd.read_csv("C:/Users/Tugce.Dogan/Downloads/Telco-Customer-Churn.csv")
df.head()
df.shape
df.info()
...
# 1. yöntem (medyan atama)
df["TotalCharges"].fillna(df["TotalCharges"].median(), inplace=True)

# 2. yöntem (aylık ödenecek miktarlarıyla totalcharge doldurulailir)
df["TotalCharges"].fillna(df["MonthlyCharges"]*df["tenure"], inplace=True)

# 3. yöntem  (11 na değişken drop edilebilir)
...
accuracy_score(y_test,y_cart_pred)
...
cart_tuned=DecisionTreeClassifier(max_depth=3 , min_samples_split= 2).fit(X_train,y_train)
print(skompile(cart_tuned.predict).to("python/code"))
y_cart_pred=cart_tuned.predict(X_test)
accuracy_score(y_test,y_cart_pred)
...
rf_cv=GridSearchCV(rf_model, rf_params, cv=5, n_jobs=-1, verbose=2).fit(X_train,y_train)
rf_cv.best_params_
rf_tuned=RandomForestClassifier(max_depth=10, max_features=5,min_samples_split=2,n_estimators=500 ).fit(X_train,y_train)
